...
export const dbSetSession = (user: DBUser | null) => {
  if (user) localStorage.setItem(KEYS.SESSION, JSON.stringify({ id: user.id }));
  else localStorage.removeItem(KEYS.SESSION);
};

// ─── ADMIN GATEWAY ──────────────────────────────────────────
export const ADMIN_ALLOWED_EMAILS = ['admin@papix.com', 'owner@papix.com'];
export const ADMIN_CREDENTIALS = { email: 'admin@papix.com', password: 'Papix2024!' };

export const isAllowedAdminEmail = (email: string) =>
  ADMIN_ALLOWED_EMAILS.includes(email.toLowerCase());
...
export const dbAdminLogin = (email: string, password: string): boolean =>
  isAllowedAdminEmail(email) && password === ADMIN_CREDENTIALS.password && isTrustedAdminNetwork();
